---@diagnostic disable: duplicate-set-field, different-requires
...
--- Normalizes a message's keys and tags to title case and extracts tags to root-level fields.
-- @lfunction normalizeMsg
-- @tparam {table} msg The message to normalize
-- @treturn {table} The normalized message with title-cased keys and extracted tags
...
-- @tparam {string} key The key to match against
-- @tparam {any} value The value to match
-- @treturn {table|nil} The first matching object, or nil if not found
...
--- Print a value to the outbox output, formatting tables and colorizing non-string types.
-- @function print
-- @tparam {any} a The value to print
-- @treturn {string|nil} The string representation of the value, or nil if HANDLER_PRINT_LOGS is active
function print(a)
...
-- Wraps ao.send and returns a table with output confirmation, receive, and onReply callbacks.
-- Prints a warning if no Target is specified.
-- @function Send
-- @tparam {table} msg The message to send. Should include a Target field
-- @treturn {table} A table with output string, receive function, and onReply callback
function Send(msg)
...
-- Wraps ao.spawn. Can be called with (module, msg) or just (msg) to use the current module.
-- @function Spawn
-- @tparam {string} [module] The module source ID. If omitted, uses ao._module
-- @tparam {table} [spawnMsg] The spawn configuration message
-- @treturn {table} A table with output string, after callback, and receive function
function Spawn(...)
...
--- Assigns a message to one or more processes.
-- Wraps ao.assign and prints a confirmation message.
-- @function Assign
-- @tparam {table} assignment The assignment containing Processes (table) and Message (string)
-- @treturn {string} A confirmation message, or an error message if assign is not implemented
...
-- Initializes the process state, normalizes the message, validates trust and assignability,
-- evaluates registered handlers via coroutine, and compiles the result.
-- @function handle
-- @tparam {table} msg The incoming message to process
-- @tparam {table} _ The environment object (may contain Process directly or nested under .env)
-- @treturn {table} The process result containing Output, Messages, Spawns, and Assignments (or Error)
